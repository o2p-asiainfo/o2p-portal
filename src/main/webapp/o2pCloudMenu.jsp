<!-- BEGIN HEADER -->
<div class="header navbar sss navbar-default navbar-static-top">
  <div class="container">
    ...
    <c:choose>
    	<c:when test="${userBean.name==null || userBean.name==''}">
      			...
      			<!-- BEGIN LOGO (you can use logo image instead of text)--> 
					...
					<a class="navbar-brand logo-v1" href="${APP_PATH}"  target="mianframe"> <img src="${APP_PATH}/resources/img/${contextStyleSpecial }/logo${localeName }_super.png" id="logoimg" alt="" height="50"> </a>
																																														
      			<!-- END LOGO --> 
    		</div>
		    <div class="navbar-collapse collapse">
		      <ul class="nav navbar-nav">
		        <li <c:if test="${navBarPageId=='Home'}"> class="active" </c:if>> <a href="${APP_PATH}"  target="mianframe"> ${local["eaap.op2.portal.index.index"]} </a> </li>
		        <li class="dropdown <c:if test="${navBarPageId=='Guide'}"> active </c:if>"> <a href="#" data-close-others="false" data-delay="0" data-hover="dropdown" data-toggle="dropdown" class="dropdown-toggle"> ${local["eaap.op2.portal.index.guide"]} <i class="fa fa-angle-down"></i> </a>
		          <ul class="dropdown-menu">
		            <li><a href="${APP_PATH}/developerGuide.shtml"  target="mianframe">${local["eaap.op2.portal.index.devGuide"]}</a></li>
		            <li><a href="${APP_PATH}/providerGuide.shtml"  target="mianframe">${local["eaap.op2.portal.index.proGuide"]}</a></li>
		            <li><a href="${APP_PATH}/partnerGuide.shtml"  target="mianframe">${local["eaap.op2.portal.index.parGuide"]}</a></li>
		          </ul>
		        </li>
		        ...
		        <span class="sep"></span>
		          <a href="${APP_PATH}/org/tologin.shtml"  target="mianframe">${local["eaap.op2.portal.index.login"]}</a> | <a href="${APP_PATH}/org/reg.shtml"  target="mianframe">Register</a>
		        </li>
		      </ul>
		    </div>
    	</c:when>
...
<%-- 			                    <li><a href="forwardRTI.jsp"  target="mianframe">${local["eaap.op2.portal.index.rtiView"]}</a></li> --%>
...
<!-- 			                    <li><a href="settlement.jsp">Settlement</a></li> -->
			        ...
			        <li class=" dropdown <c:if test="${navBarPageId=='UserInfo'}">active </c:if>"> <span class="sep"></span> <a class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-delay="0" data-close-others="false" href="#"> ${userBean.name} <i class="fa fa-sort-down"></i></a>
			          <ul class="dropdown-menu s-user-box">
			            ...
			            <li><a href="${APP_PATH}/org/logout.shtml" onclick="window.parent.location.href = 'default.shtml''"><i class="fa fa-sign-out"></i>${local["eaap.op2.portal.index.signOut"]}</a> </li>
			          </ul>
			        </li>
			      </ul>
			    </div>
